...
python3 - "$TSV" "$HERE/probe_results.json" "$TKC" <<'PY'
import sys, json, subprocess, datetime
tsv, out, tkc = sys.argv[1:4]
rows = []
...
sha = subprocess.run(["git","-C",tkc.rsplit("/",1)[0],"rev-parse","HEAD"],capture_output=True,text=True).stdout.strip()
...
json.dump({"story":"131.2","tkc_version":ver,"tkc_sha":sha,"generated":datetime.date.today().isoformat(),"summary":summary,"results":rows}, open(out,"w"), indent=1)
