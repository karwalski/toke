...
def dummy_arg_symbols() -> set[str]:
    ...
    if not DUMMYARG_GEN.exists():
        print(f"ERROR: {DUMMYARG_GEN.relative_to(REPO_ROOT)} not found; it is "
              f"generated by scripts/gen_stdlib_decls.py and the 135.17 arity "
              f"check cannot tell the zero-argument convention from drift "
              f"without it.")
        raise SystemExit(2)
    syms = set(re.findall(r'"(\w+)"', DUMMYARG_GEN.read_text(errors="replace")))
    require_population("g_stdlib_dummy_arg", len(syms), 10,
                       str(DUMMYARG_GEN.relative_to(REPO_ROOT)))
    return syms
