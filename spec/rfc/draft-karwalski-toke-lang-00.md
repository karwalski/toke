...
Abstract
   ...
   and validate the core thesis.  The default syntax
...
Table of Contents
   ...
   5.1.  The 59-Character Alphabet .................................  8
...
## 0.  Document Status and Errata (2026-09-19)
   ...
   o  The character-set count is **59** — RESOLVED 2026-09-19 by story
      132.14, and Section 5 is corrected in place.  This draft said 56
      while its own table listed 21 symbols and totalled 20; the
      language specification, the glossary and the positioning brief
      said 55 (19 symbols).  Neither reconciled with the reference
      compiler.  `src/lexer.c` governs: in PROFILE_DEFAULT it rejects
      exactly eight printable ASCII characters in structural position
      (' , ? [ \ ] _ backtick) and accepts every other symbol, giving
      26 lowercase + 10 digits + 23 symbols = 59.  Both earlier counts
      omitted `%` and `&`, which are live operators; the 55 count also
      omitted `^` and `~`, which story 114.8 assigned to bitwise XOR
      and bitwise NOT.  The single source for this and every other
      project-scale count is now `docs/metrics-baseline.md` § Project
      facts, regenerated by `scripts/verify_project_facts.py`.
...
## 2.  Conventions and Terminology
   ...
   Default syntax (normative):
      The lowercase, sigil-based variant that constitutes the toke
      language as
...
## 5.  Character Set Specification
   ...
   toke uses exactly 59 structural characters.  No character outside
   this set SHALL appear in toke source in a structural position.
   Arbitrary UTF-8 is permitted within string literal content.

   This section was corrected on 2026-09-19 by story 132.14.  The
   March 2026 draft stated 56 and its table listed 21 symbols while
   totalling 20, so the figure did not reconcile even with itself; the
   language specification and the positioning brief stated 55.  Neither
   matched the reference compiler.  The count below is derived from
   `src/lexer.c`, which is the ground truth: in PROFILE_DEFAULT the
   lexer rejects exactly eight printable ASCII characters in structural
   position with diagnostic E1003 — ' , ? [ \ ] _ and backtick — and
   accepts every other symbol.  Reproduce with
   `python3 scripts/verify_project_facts.py --probe`.

### 5.1.  The 59-Character Alphabet
...
```
   ...
   o  Excluded symbols: backtick, backslash, single-quote, comma,
      question-mark.  Backslash is legal inside a string literal as an
      escape introducer, but not in a structural position.

   o  Underscore (_): excluded by the no-underscore naming rule
      (story 113.2a).  An identifier beginning with _ is E1003.

   o  Hash (#): not a member of the alphabet.  The lexer accepts it
      only to emit W1020 ("Python comment detected") and skip the line,
      which is error recovery, not language surface.

   The characters ^ and ~ ARE assigned.  Story 114.8 gave them bitwise
   XOR and bitwise NOT respectively, and `%` (modulo) and `&`/`&&`
   (bitwise and, short-circuit and) are likewise live operators.  The
   March 2026 draft described all four as absent or reserved; that text
   is retired.  Nothing in the alphabet is reserved-but-unusable today.
...
### 5.2.  Legacy Profile (80 Characters) — Historical
   ...
   default syntax is fully deterministic and
   implemented in the reference compiler via --profile1 and --profile2
   flags.

   The legacy profile uses 26 lowercase + 26 uppercase + 10
   digits + 24 symbols = 86 structural characters (corrected
   2026-09-19, story 132.14: `tkc --legacy` rejects only $ @ ' , ?
   backslash and backtick; the "19 symbols / 81" figure was never
   measured).  It is documented
...
```
   ...
   ^       Bitwise XOR (assigned by story 114.8)
   ~       Bitwise NOT (assigned by story 114.8)
   %       Modulo
   &       Bitwise and; && is short-circuit logical and
```
...
```
   ...
   Compiler frontend: approximately 7,700 lines of C with zero
   external dependencies (excluding LLVM as a build dependency);
   `src/` in full is about 29,700 lines of C across the frontend,
   backend, formatter and tooling (measured 2026-09-19, story 132.14;
   `wc -l src/*.c`).  The "3,700 lines" figure in this draft was an
   April 2026 snapshot and is superseded.
...
### 15.4.  Purpose-Built Tokenizer
   ...
   mechanically translated to the default syntax.
...
## 17.  Project Stages
   ...
   "default syntax" described in Section 5.
...
### 17.1.  Stage 1 — Falsification (Months 1-8)
   ...
   o  Compiler: 3,700 lines of C, 90/90 conformance tests, 9/9 e2e
      ...
      and ARM64.  [April 2026 snapshot.  As of 2026-09-19 the
      conformance suite is 222 YAML cases plus 6 shell scripts and
      `src/` is about 29,700 lines of C — see
      `docs/metrics-baseline.md` § Project facts.]
...
### 17.2.  Stage 2 — Language Viability (Months 8-14)
   ...
   Objective: transition to the default syntax, extend
...
```
   ...
   (April 2026 column; the suite is 228 cases and src/ ~29,700 LOC
...
## 23.  Open Questions
   ...
   **Reserved characters ^ and ~ — CLOSED (story 114.8)**

      Assigned to bitwise XOR and bitwise NOT, as the candidates
      anticipated here.  No profile version change was required.  The
      alphabet now holds no reserved-but-unusable character; see
      Section 5.1 as corrected by story 132.14.
