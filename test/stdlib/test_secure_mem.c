/*
 * test_secure_mem.c — Unit tests for the std.secure_mem C library.
 *
 ...
 *       src/stdlib/secure_mem.c test/stdlib/test_secure_mem.c \
 *       -lpthread -o /tmp/test_secure_mem && /tmp/test_secure_mem
 *
 * Story: 72.3.4
 */
...
#include "../../src/stdlib/secure_mem.h"
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(64, 300);
    ...
    TkSecureBuf buf2 = tk_secure_mem_alloc(32, 0);
    ...
    TkSecureBuf bad = tk_secure_mem_alloc(0, 10);
    ASSERT(bad.id[0] == '\0',     "alloc(size=0): returns empty handle");

    /* Wipe to clean up. */
    tk_secure_mem_wipe(buf);
    tk_secure_mem_wipe(buf2);
}
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(128, 60);

    int wrote = tk_secure_mem_write(buf, "hello-secret");
    ASSERT(wrote == 1, "write: returns 1 on success");

    TkSecureReadResult r = tk_secure_mem_read(buf);
    ASSERT(r.is_none == 0,              "read: is_none=0 after write");
    ASSERT_STREQ(r.value, "hello-secret", "read: value matches written data");
    free(r.value);

    /* Overwrite with a different value. */
    tk_secure_mem_write(buf, "updated");
    TkSecureReadResult r2 = tk_secure_mem_read(buf);
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(64, 60);
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(64, 300);
    tk_secure_mem_write(buf, "to-be-wiped");

    int wiped = tk_secure_mem_wipe(buf);
    ...
    int wiped2 = tk_secure_mem_wipe(buf);
...
static void test_write_wiped(void)
{
    ...
    int wrote = tk_secure_mem_write(buf, "ghost");
...
static void test_sweep(void)
{
    ...
    TkSecureBuf b1 = tk_secure_mem_alloc(32, 1);
    TkSecureBuf b2 = tk_secure_mem_alloc(32, 1);
    TkSecureBuf b3 = tk_secure_mem_alloc(32, 300);  /* longer TTL */

    tk_secure_mem_write(b1, "expire1");
    tk_secure_mem_write(b2, "expire2");
    tk_secure_mem_write(b3, "survivor");

    /* Before expiry, sweep should free nothing (or at most 0 of these 3). */
    int32_t before = tk_secure_mem_sweep();
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(64, 1);  /* 1-second TTL */
    tk_secure_mem_write(buf, "expires-soon");

    /* Immediately readable. */
    TkSecureReadResult r1 = tk_secure_mem_read(buf);
...
{
    int avail = tk_secure_mem_is_available();
...
{
    TkSecureBuf buf = tk_secure_mem_alloc(64, 60);
    int wrote = tk_secure_mem_write(buf, NULL);
...
static void test_empty_handle(void)
{
    ...
    TkSecureReadResult r = tk_secure_mem_read(empty);
    ASSERT(r.is_none == 1, "read(empty handle): is_none=1");

    int wrote = tk_secure_mem_write(empty, "ghost");
    ASSERT(wrote == 0, "write(empty handle): returns 0");

    int wiped = tk_secure_mem_wipe(empty);
...
static void test_multiple_buffers(void)
{
    ...
    for (i = 0; i < MULTI_N; i++) {
        bufs[i] = tk_secure_mem_alloc(64, 60);
        snprintf(expected[i], sizeof(expected[i]), "secret-%d", i);
        tk_secure_mem_write(bufs[i], expected[i]);
    }

    /* Verify each buffer returns its own value. */
    int all_ok = 1;
    for (i = 0; i < MULTI_N; i++) {
        TkSecureReadResult r = tk_secure_mem_read(bufs[i]);
    ...
    for (i = 0; i < MULTI_N; i++) tk_secure_mem_wipe(bufs[i]);
}
...
static void *thread_write(void *arg)
{
    ...
    a->result = tk_secure_mem_write(a->buf, a->data);
    return NULL;
}

static void test_concurrent_write(void)
{
    TkSecureBuf b1 = tk_secure_mem_alloc(64, 60);
    TkSecureBuf b2 = tk_secure_mem_alloc(64, 60);
    ...
    TkSecureReadResult r1 = tk_secure_mem_read(b1);
    TkSecureReadResult r2 = tk_secure_mem_read(b2);
    ...
    tk_secure_mem_wipe(b1);
    tk_secure_mem_wipe(b2);
}
