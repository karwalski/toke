...
| `conformance_cases_shell` | **51** | `ls test/conform/*.sh \| wc -l` |
| `conformance_cases_total` | **276** | the two rows above; `make conform` runs both |
...
| `stories` | **2,666** | `grep -oE '^\| [0-9]+\.[0-9]+[a-z0-9.]* \|' docs/progress.md \| sort -u \| wc -l` — *live, not gated: moves with every tracker commit, re-derive before citing* |
