...
| `stdlib_modules` | **68** | `ls stdlib/*.tki \| wc -l` |
| `conformance_cases_yaml` | **225** | `find test -name '*.yaml' \| wc -l` (grammar 98, diagnostics 92, lexical 35) |
| `conformance_cases_shell` | **52** | `ls test/conform/*.sh \| wc -l` |
| `conformance_cases_total` | **277** | the two rows above; `make conform` runs both |
...
| `stories` | **2,669** | `grep -oE '^\| [0-9]+\.[0-9]+[a-z0-9.]* \|' docs/progress.md \| sort -u \| wc -l` — *live, not gated: moves with every tracker commit, re-derive before citing* |
