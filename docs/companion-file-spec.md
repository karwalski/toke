# Companion File Format Specification
...
**Must include:**
- The module name (the value from `m=` or `M=`).
...
Describes every type declaration (`t=` or `T=`) in the source file.
...
Describes every function declaration (`f=` or `F=`) in the source file. This is the most detailed section. Each function gets its own `###` subsection.
