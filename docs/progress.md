...
| 123.10 | Parser: brace-terminated single-return body → spurious E2003 | planned | P2 | **New 2026-07-07 (found reproducing 123.5).** A function whose body is a single-statement return of a brace-terminated literal — e.g. `f=mk():$b{ <$b{x:5} }` — fails with `E2003 "missing semicolon"` at the **next** `f=` definition: the struct-literal's `}` is mis-associated with the function body's `}`. Blocks the common factory-function shape (`f=mk():$T{ <$T{...} }`). Isolate in `src/parser.c` (return-statement terminator vs struct-literal `}`); add a regression. Multi-statement bodies and non-brace returns are unaffected. |
