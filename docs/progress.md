...
| 102.22 | Compiler bug: LLVM IR codegen type mismatch (i8* vs i64*) | planned | | Some valid programs produce broken IR: `%t6 defined with type i8* but expected i64*`. Likely in array/string operations where the ABI wrapper expects i64 but gets a pointer. Investigate getelementptr emission in llvm.c for collection/string operations. |
