...
| 124.4-testcaps | Grant capabilities in standalone stdlib test harnesses | planned | P1 | The 124.4 deny-by-default flip makes the standalone `test-stdlib-*` binaries that call gated fs/net/os sinks abort at runtime with CAP001 (e.g. `test-stdlib-db` → 'fs.read required'). Add `tk_cap_init(argc,argv)` + `--allow-all` (as `test_ambient.c` does) to the affected harnesses, or grant per-test. Not a product bug — a test-harness gap from the flip. |
