...
| 124.4 | **Wave A:** ADR-0010 capability subsystem (umbrella → 124.4a–g) | **PARKED — needs design pass** | **P0** | Deny-by-default capabilities; grants declared out-of-band so the source grammar is untouched. **Parked 2026-07-04:** greenfield — `config.c` only parses compiler limits today, no capability infra exists. This is a large cross-cutting subsystem (`--allow-*` flag parsing + a **compiler→runtime capability-threading mechanism** + enforcement at every net/fs/exec/env stdlib boundary + the `tkc.toml` manifest + `.tkc.md` companion). The threading mechanism is a genuine architecture decision (compiler-embedded capability-bitmask global vs runtime manifest read vs env) that shapes 124.4a–g — **flag Opus / owner design confirmation before implementing.** Not an autonomous-loop tick; do as a dedicated attended increment. Decomposed into 124.4a–g. |
