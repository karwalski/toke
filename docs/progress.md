...
| ID | Story | Status | Priority | Notes |
|----|-------|--------|----------|-------|
| 123.1 | Consolidate the v0.4 normative spec (single source) | planned | **P0** | Fold the 141-line v0.4 amendment into one normative doc (`toke-spec-v0.4.md`); regenerate the `grammar.ebnf` header (13→14 keywords; drop "strict LL(1)" → point at Appendix A); re-stamp `memory-model.md`; **rewrite/retire `known-limitations.md`** (it wrongly claims no closures/options/unit-enums — all now exist). Ties 116.13-H1. Gate: `grep "strict LL(1)\|Keywords (13)" docs/spec/grammar.ebnf` empty; the "not supported" claims spot-check as compiling. |
| 123.2 | One source of truth for project status | planned | **P0** | Re-sync `/tk/progress.md` (5 weeks stale, says "Gate 2 ON HOLD") and `PROJECT_STATUS.md` to the authoritative `toke/docs/progress.md`; correct the "Gate 2 PASS" line to honest compile-vs-functional framing (100% compile / ~56% functional). Add a governance rule: dashboards are regenerated from the detailed tracker, never hand-edited independently. (R014 in the wild.) |
| 123.3 | Honest metrics baseline + methodology | planned | P1 | Publish the anchor numbers for all future claims: `--min` token-efficiency (Epic 116/B machinery), functional-correctness %, compile-fail count — with methodology. No external/academic efficiency claim ships before this (mitigates R008 benchmark-representativeness, R015 peer-review). |
...
| 123.5 | Fix known codegen correctness bugs (root of trust) | planned | **P0** | These corrupt the corpus → the model. Fix: struct-field map access → SIGBUS (`is_map_var()`, 56.10.4); `as $str` on ints → garbage `inttoptr`; unreliable `\(expr)` string interpolation for non-str/int. Add regression tests; update `known-limitations.md` as each closes. |
