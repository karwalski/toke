...
| 124.4 | **Wave A:** ADR-0010 capability subsystem (umbrella → 124.4a–g) | **in progress** (mechanism + a/b/c[primary] DONE; enforcement demonstrated end-to-end, default still ALLOW_ALL until 124.4g) | **P0** | Deny-by-default capabilities; grants declared out-of-band so the source grammar is untouched. **Threading mechanism DECIDED 2026-07-06 (owner-confirmed):** option (a) — the compiler bakes the required grant set into the binary + the program accepts runtime `--allow-*` flags + a runtime broker gates each sink; effective grants = baked ∪ runtime. Chosen over (b) runtime `tkc.toml` read (binary not self-describing/portable) and (c) env var (ambient/spoofable). Precedent: the compiler already bakes `max_iters` inline (`llvm.c`), and `tk_runtime_init(argc,argv)` is the guaranteed startup hook. Decomposed into 124.4a–g. |
