...
| 114.19 | **DONE** | 19a/19b were already non-segfaulting. 19c (`@(a)+b` unwrapped-scalar append) now a **compile error**: a codegen guard in the `+` dispatch rejects array+plain-i64-scalar (the type checker can't — ident RHS infers TY_UNKNOWN). Also fixed a latent bug it exposed — codegen-phase DIAG_ERRORs were silently ignored; `emit_llvm_ir` now returns -1 when `diag_error_count()>0`. Verified zero new corpus build failures. |
