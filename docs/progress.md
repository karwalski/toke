...
| 116.7 | **B** — idiom standard + `--min` formatter + idiom judge | **B1+B3 DONE; B2 pending** | P1 | **B1 ✅** `docs/spec/idiom-v0.4.md` — normative minimal-code idiom (expr-if/`&&`/stdlib/interpolation/chaining/no-dead-mut) with tokenizer-measured reductions (stdlib-over-hand-rolled −44% headline). **B3 ✅** implemented the empty `toke-model/corpus/judge/qwen_judge.py` as a **deterministic rule-based idiom scorer** (mut-flag-if/flag-soup/hand-parser/nested-concat) and wired it into `validate/quality.py` as a **hard gate** (idiom<0.6 rejected, per `judge/rubric.md`); DAT-003→0.55 reject, ~29% of a 300-file sample would reject (matches the audit). Pushed `toke-models@feat/116-idiom-judge-clean` (main has a pre-existing >100MB `train.jsonl` blocking direct push). **B2 ⏳ (own sprint):** `fmt.c` is broadly **stale for v0.4** — `--fmt` emits old `[]` (not `@()`), no `$` struct sigils, and truncates expr-`if`/match. Needs a focused v0.4 formatter overhaul (structs `$`, arrays/maps `@()`, expr-`if`, `mt`, index→`.get`, `$`-type sigils) with round-trip stability, **then** add the `--min` single-line canonical form (training target + tokenizer input) + an idiom lint pass. |
