...
| 126.8 | **Residual failures — 7/1460 (library at 1453/1460 = 99.5%).** **AIA-044 FIXED** (`8...`): `s.fromint` resolves to `tk_str_from_int`, missing from the interp str-wrapper set → `\(s.fromint(x))` rendered a pointer; added it (+repeat/toupper/tolower). Full-1460 delta: 8 recovered, 0 regressed. **Remaining (documented, not quick/safe fixes):** (a) **2 json-API-drift** — AIA-100/105 use stale `json.getarr`/`getobj`/`getint` (not in `.tki`) + `specs.get(i).raw` on json-array elements; need program migration to `json.arr`/`json.str` or deep `[Json]`-element field typing. (b) **5 FILE_DEP** — SEC-071/DEV-049/SYS-081/SYS-125/DEV-109 read input files the manifest `test_cases` omit (programs likely correct); also `f.read` on a missing file returns NULL → downstream `s.split` SIGSEGVs (robustness: a nil-string trap or `str!FileErr` handling). Provide fixtures or tag fixture-dependent. | in progress | P2 |
