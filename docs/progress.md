...
| ID | Story | Status | Prio | Notes |
|----|-------|--------|------|-------|
| 116.1 | **A1** — expression-oriented `if`/`match` | planned | P1 | Biggest token lever. `if`/`el` yield a value (bind/return/arg position), add `el if`; confirm `match` (`mt`) as first-class expression + fix `<`-in-match-arm codegen (`ret null` gap). Kills the `let x=mut.0;if(c){x=a}el{x=b}` pattern that dominates the corpus. Files: `parser.c` (if/match into the expr precedence chain), `types.c` (branch-result unification), `llvm.c` (phi), spec, `grammar.ebnf`. **A1a research:** `lp`-as-value vs stdlib combinators (recommend combinators). |
| 116.2 | **A2** — short-circuit `&&` / `\|\|` | planned | P1 | Additive, non-breaking; kills flag-soup boolean logic (today "use a flag"). Lexer + parser precedence (below comparison) + `llvm.c` short-circuit branches + `grammar.ebnf`. First increment. |
| 116.3 | **A3** — split `=` (assign) vs `==` (equality) | planned | P1 | Breaking. Resolves the worst non-LL(1) spot (unbounded loop-init scan) and aligns with LLM priors. `TK_EQEQ`; `--migrate`. **A3a research:** measure `==` net token cost with the tokenizer. |
| 116.4 | **A4** — eliminate parser backtracking + multi-token lookahead | planned | P1 | Strict LL(1): rework the postfix production (`.get`/index/method) to single-token lookahead, no save/restore (`parser.c:664-679,724-736`); remove 3-token peeks (`409,526,1090-1091`) by grammar restructuring. **A4a research:** whether `[]` indexing is the only clean fix (vs the `@()` decision). |
| 116.5 | **A5** — FIRST/FOLLOW proof + machine-readable grammar | planned | P1 | Write the missing normative LL(1) appendix (FIRST/FOLLOW + parse table) for v0.4; automated grammar-vs-parser conformance test; emit EBNF/Lark/GBNF as the single constrained-decoding source (feeds 116.11). |
| 116.6 | **A6** — stdlib parsing/scan helpers | planned | P2 | Typed `json.parse`/accessors, `csv`, `str` scan/fields so hand-rolled char state-machines (EDU-001/DAT-003/AIA-001) disappear. **A6a research:** mine corpus for hand-rolled patterns, rank stdlib gaps. (114.18 array CoW prerequisite done.) |
| 116.7 | **B** — idiom standard + `--min` formatter + idiom judge | planned | P1 | `docs/spec/idiom-v0.4.md` (the one right way, with token counts); extend `fmt.c` with a `--min` single-line canonical form (training target + tokenizer input); extend `lint-rules-v1.md` for the real anti-patterns; implement the empty `toke-model/corpus/judge/qwen_judge.py` and make idiom a **hard gate** in `validate/quality.py`. |
