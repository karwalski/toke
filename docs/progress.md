...
| 129.3 | Sweep: library, test programs, examples, fixtures | planned | **P0** | Same rubric over the 1460 verified library programs, the wider `toke-test-programs` set, canonical doc examples (extend `scripts/check_doc_examples.py` with the structural metrics), and regen fixtures. Preserve Epic 126's preserve-outputs constraint. Report where these assets diverge in style from the regen corpus — training data must be *consistently* idiomatic across sources. |
