...
| 124.3 | **Wave A:** ADR-0011 runtime parts | **in progress** (HTML attr-context escaping DONE) | **P0** | **DONE 2026-07-04:** **attribute-context escaping** — `render_attrs` (`html.c`) wrote attribute values *raw* into the quoted attribute, so a `"` in a value broke out and injected markup/event handlers (XSS). Now routes values through `html_escape` (`& < > " '`). Verified: new `test_html.c` case — a `"><script>` href is neutralized (no raw `<script>` in output; `"`→`&quot;`) + all existing html tests pass (`make test-stdlib-html`). **Remaining:** delete `spawn_shell` wrappers + argv-only guarantee (`process_glue.c:18`); MySQL param-bind bug (DAT-03); full BODY/URL/JS/CSS context escaper in `template.c` + point `tpl.render`/ooke `{=x=}` at escape-on; stop `CMARK_OPT_UNSAFE`; sweep the live ooke site. |
