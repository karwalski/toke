...
| 124.4c2 | Route the secondary/non-`_w` sinks | planned | **P0** | **Deferred from 124.4c (logged, not silent):** the sinks that bypass the `_w` glue or are lower-traffic — `http.c`/`http2.c` server surface (inlined `socket`/`bind`/`listen`/`accept`, `http_serve*`) → NET; the **real** `tls.c` connect/listen (TK_OPENSSL path, distinct from the gated stubs) → NET; raw `os.c` syscalls (`tk_os_open`/`read`/`write`/`unlink`/`rename`/`mkdir`/`rmdir` → FS_READ/WRITE, `setenv` → ENV_WRITE); `db_glue.c` (open/connect → NET or FS by backend); `mdns`/`keychain`/`clipboard`/`sse`/`proxy`. Must all be gated before the 124.4g flip (an ungated sink = a bypass). |
