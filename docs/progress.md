...
| 124.4d | Structured missing-grant diagnostic | **partly DONE (CAP001 shipped in 124.4a)** | **P0** | The runtime `CAP001` trap already names the missing grant + the flag/tkc.toml that grants it (`tk_cap_deny`, `capabilities.c`). **Remaining:** decide whether the repair loop wants a machine-parseable (JSON) form vs the current human line (ADR-0001 §4 is about *compiler* diagnostics; this is a *runtime* trap) — likely add an optional structured line. Low risk. |
