...
| 124.0a | Fix the two silent-wrong-value codegen bugs | planned | **P0** | Executes **123.5**: anonymous-function miscodegen (`fn(x){x+1}(7)`→7; captured vars→0) and the option `$none` match arm (returns 0, not the arm value). Both in `src/llvm.c`. Regression tests must compile **and run** (exit-code), not just `--check`. |
