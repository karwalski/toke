...
| 75.10 | Standalone binary compilation — only emit used extern declarations | done | 2026-05-11 | **P0** llvm.c: replaced 171 unconditional `declare` statements with two-pass emission. IR body emitted to tmpfile first, then scanned for referenced `@symbol` names. Only used declarations emitted. Minimal program: 5 declarations (was 171). Standalone binary links with just tk_runtime.c + args.c + str.c — no web glue, db, collections, or http needed. |
