...
| 124.3 | **Wave A:** ADR-0011 runtime parts | **in progress** (HTML attr-escaping + shell-wrapper removal DONE) | **P0** | **DONE 2026-07-04:** (1) **attribute-context escaping** — `render_attrs` (`html.c`) wrote attribute values *raw* into the quoted attribute, so a `"` broke out and injected markup/event handlers (XSS). Now routes values through `html_escape` (`& < > " '`); verified new `test_html.c` case (`"><script>` href neutralized) + all html tests pass. (2) **shell-wrapper removal / argv-only** — `process.exec`/`spawndetached`/`readlines` ran the command string via `sh -c` (injection sink); replaced with a non-shell tokenizer (`spawn_cmdstr` in `process_glue.c`) that splits on whitespace (quoted segments intact) and `execvp`s directly, so `;`/`\|`/`$()`/backticks are never interpreted. Verified: `exec("echo hello")` still works (`test_process_full`), injection probe (`exec("echo a; touch f")` creates no file), new `test/standalone/test_process_noshell.tk` (`;` treated literally), conform 180/0. Note: shell operators (`&&`, `\|`) in `exec` strings are now literal (intended). **Remaining:** MySQL param-bind bug (DAT-03, needs mysql — PARK); full BODY/URL/JS/CSS context escaper in `template.c` + point `tpl.render`/ooke `{=x=}` at escape-on; stop `CMARK_OPT_UNSAFE`; sweep the live ooke site. |
| 124.4 | **Wave A:** ADR-0010 capability subsystem (umbrella → 124.4a–g) | planned | **P0** | Deny-by-default capabilities; grants declared out-of-band so the source grammar is untouched. Decomposed into 124.4a–g. |
