...
| 126.5 | **Codegen: nested int-array element mis-typed `TY_STR` → strcmp crash.** `expr_llvm_type` trusted a spurious `TY_STR` on a nested subscript (`dist.get(i).get(j)`), so `dist.get(i).get(j) >= 999999` lowered to `strcmp` on a non-pointer i64 → SIGSEGV. **PARTIAL DONE 2026-07-08** (`7eb495e`): direct-subscript comparison now numeric (`test_nested_arr_compare.tk`; conform 183/0; diff-codegen 0). **The 6 corpus crashers (SCI-019/082, MSG-079, AIA-091/124, SEC-071) still SIGSEGV** — a *deeper*, hard-to-minimise crash: Floyd-Warshall `dist=newdist` rebuild (shared inner rows) + full output-loop read; every reduction I tried runs, only the full programs crash. Needs dedicated IR-level bisection. | in progress | **P0** |
