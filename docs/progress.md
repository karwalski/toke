...
| 114.41 | Codegen: error-union error path with a typed-payload sum-type error is broken | planned | **P1** | Returning a sum-type error from a `T!$E` function and matching it does **not** work: `f=fail():$i64!$e{<$e{$bad:"m"}}` then `mt fail(){$ok:v ..; $err:e mt e{$bad:msg ..}}` emits invalid IR (`use of undefined value '%rm_err1'`); even when it compiles (arms calling helpers, as in the first tkgrep rewrite) the **error arm never fires at runtime** (returns rc 0, prints nothing) because the zero-sentinel encoding drops the Err payload and mis-signals ok-vs-err. Related to 114.13 (the same zero-collision). This **blocks the error-union/typed-error pattern the tutorials teach** — `tkgrep` was reworked to avoid error unions entirely (pre-check args + `file.exists`, unwrap `file.read` with a default `mt {$ok:v v;$err:e ""}`). Until fixed, examples can't faithfully demonstrate `T!$E`. |
