...
| 102.14 | Implement missing stdlib C glue functions | done | 2026-05-25 | io.readln() had no C implementation — declared in .tki but missing from io_glue.c. Root cause of 0% test-pass on workers and possibly the 8% Gate 2 functional result. Added tk_io_readln_w(). Audit ALL .tki declarations vs C glue for other missing functions. |
| 102.15 | Re-evaluate Gate 2 functional correctness with io.readln fix | planned | | The ~8% functional Pass@1 from Gate 2 may have been caused by missing io.readln() C glue, not by the model generating wrong code. Re-run the Gate 2 evaluation with the fixed stdlib. If functional correctness is significantly higher, update: gate2-decision.md, whitepaper, website timeline, README, all references to "8% functional". This could change the entire project narrative. |
