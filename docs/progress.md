...
| 124.2e | Pre-existing crash: `test/standalone/test_crypto_full.tk` SIGSEGVs | backlog | P2 | Found 2026-07-04 during 124.2a diff-gate analysis: this crypto test crashes every run (baseline signature was already `signal`), non-deterministically SIGSEGV vs SIGBUS, with empty stdout — a pre-existing memory bug on this branch, unrelated to the bounds-check (the program does no array indexing). Investigate the crypto stdlib path it exercises. |
