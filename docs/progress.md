...
| ID | Story | Status | Priority | Notes |
|----|-------|--------|----------|-------|
| 129.1 | Audit harness + codified quality rubric | planned | **P0** | GATES the epic. One harness that sweeps any asset set and emits per-program results: compile (`--check` + full build) under tkc 2.8.0, test-case execution where (input, expected-output) pairs + a suitable harness exist (reuse `run_shard.py validate` machinery + `check_one.py`), and **structural metrics** — max conditional nesting depth, function length/count, tokens (readable + `--min`). Codify the rubric with numeric thresholds (e.g. nesting depth ≤3, function-length ceiling, functions-per-program floor for non-trivial tasks) so "compact + small functions + no deep ifs" is measurable, not aesthetic. Rubric doc: `toke-corpus/regen/quality_rubric.md`. |
| 129.2 | Sweep: v0.4 regen corpus, shards 00–11 | planned | **P0** | Independent re-audit of every *accepted* record across all 12 shards (~23.4k) with the 129.1 harness — trust nothing banked, re-verify compile + test-run + structural metrics. Per-shard/per-category report; every failure flagged in the shard ledger and queued (129.5). Also spot-audits the acceptance pipeline itself: any record that passed `validate` but fails now indicates a harness gap — file a story. |
