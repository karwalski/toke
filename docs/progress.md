...
| ID | Story | Status | Prio | Notes |
|----|-------|--------|------|-------|
| 114.35 | Linker: stdlib `_w` glue not in standalone link set for ~13 modules | planned | **P1** | `i=toon:std.toon; toon.enc(...)` (and `canvas`, `chart`, `dashboard`, `dataframe`, `html`, `i18n`, `image`, `router`, `svg`, `template`, `ws`, `analytics`) pass `--check` then fail link: `Undefined symbols: _tk_toon_enc_w` etc. Same root cause as **114.31/114.25** — the `_w` wrappers live in `tk_web_glue.c` / a `*_glue.c` not in the module's standalone `c_files`, so a bare import doesn't link them. Every stdlib **doc example** for these modules currently fails to build. Fix pattern = 114.31 (move/declare glue into the module's deps closure). |
