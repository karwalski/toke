...
| 116.13 | **H** — v0.4 spec + documentation sweep (audit) | planned | **P0** | The A-workstream v0.4 changes (`=`/`==` split, expression-`if`, `&&`/`||`, backtrack-free grammar, `str.fields`) are **not propagated** to the normative spec or docs. Audit found: spec is still **v0.3** (no v0.4 doc); **13/76 doc-example `.tk` files fail to compile** under v0.4; ~77 stale `=`-equality hits in `docs/`, ~32 in `toke-website/templates`, ~12 in tutorials, ~8 in `toke-spec/`; `docs/guide/04-collections.md:213` explicitly documents the **now-false** `=`-equality rule; `design.md`/`why.md`/whitepaper assert "if is not an expression", "= is equality", "one token lookahead". **Scope:** (a) `toke-spec-v0.4.md` bump + ADRs for breaking A1/A3/A4; (b) byte-safe `migrate_eq.py` sweep of all doc/tutorial/website/spec code samples → `==`; (c) fix false prose statements; (d) website v0.4 syntax pass + redeploy; (e) wire `make check-docs-examples` (target 0/76) as a CI gate. |
