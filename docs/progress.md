...
| 102.27 | Build fail: FIN-005/FIN-032 need str.substr and struct field access | planned | | FIN-005 uses s.substr/s.set on arrays. FIN-032 uses struct types (t=Rate{from:$str;to:$str;rate:$f64}) and struct field access. Both need deeper stdlib/codegen work. |
| 102.28 | Compile fail: E4070 mutability — model uses immutable where mut needed | planned | | 24 programs write `let x=0; x=x+1` instead of `let x=mut.0; x=x+1`. Repair prompt says this but model ignores it. Need: (1) stronger prompt emphasis, (2) detect E4070 in repair loop and auto-insert mut., (3) consider compiler auto-fix suggestion in diagnostic. |
| 102.29 | Codegen bug: float arrays — double vs i64 type mismatch | planned | | 4 unique SCI programs (SCI-003, SCI-012, SCI-068, SCI-126) fail to link. Pattern: arrays storing $f64 values — .get()/.set()/.push() on float arrays emit `double` where `i64*` expected. Root: arrays are i64* internally, float values need bitcast. Fix in llvm.c where array element store/load handles float types. |
| 102.30 | Compile fail: E2002 "unexpected token 'mut'" — mut. vs mut space | planned | | 16 programs write `let x=mut 0` (space) instead of `let x=mut.0` (dot). The `mut.` syntax is unintuitive for LLMs. Options: (1) accept `mut ` as alias for `mut.` in lexer, (2) stronger prompt emphasis with explicit wrong/right examples, (3) compiler W-code suggestion "did you mean mut.?". |
...
| 102.32 | Worker: classify errors in repair loop, halt if <10% success per category | planned | | Track error patterns during repair (E4070, E2002, codegen, runtime). If success rate drops below 10% for a category after 20 attempts, pause that category and report. Prevents burning credits on categories the compiler can't handle. |
...
| 102.35 | Audit: retest all 193 buildable programs for functional correctness | planned | | 107 solutions + 86 recovered failures = 193 buildable programs. Run each with test inputs from requirements.yaml. Report: correct output, wrong output, crash. Add float tolerance (±0.01). This is free — no API calls, just local testing. |
