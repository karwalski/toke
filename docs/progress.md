...
| 116.13-H2 | Migrate standalone `.tk` doc examples | planned | **P0** | `migrate_eq.py --write` over `docs/examples/*.tk` (13/76 fail today). Gate: `make check-docs-examples` → **0/76 fail**. |
| 116.13-H3 | Migrate embedded `.md`/tutorial/spec samples + fix false prose | planned | **P0** | Markdown-fence-aware wrapper around `migrate_eq.py` (508 ` ```toke ` blocks): migrate equality `=`→`==` in `docs/**/*.md`, `docs/tutorials/`, `toke-spec/`. Fix false prose: `guide/04-collections.md:213`, `about/design.md`, `about/why.md`, `whitepaper/toke-research-language.md`. Fragments that don't parse standalone: wrap-and-migrate or hand-fix, logged. |
...
| 116.13-H5 | `check-docs-examples` CI gate | planned | P1 | Make the harness fail the build on any non-compiling `docs/examples/*.tk` (target 0/76); add to the conform/CI path so docs can't silently drift from the compiler again. |
