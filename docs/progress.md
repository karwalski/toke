...
| ID | Story | Status | Priority | Notes |
|----|-------|--------|----------|-------|
| 124.0 | **Wave 0 (BLOCKING):** fix the two silent-wrong-value codegen bugs | planned | **P0** | Executes **123.5** — anonymous-function miscodegen (`fn(x){x+1}(7)`→7) and the option `$none` arm (→0). Root of trust: a compiler that emits wrong values cannot generate corpus. Add a **differential-test harness** over the Epic 118 corpus (123.6) as the safety net for every subsequent codegen change. **Gates all codegen-adjacent work (124.2, 124.6, ADR-0014).** |
...
| 124.4 | **Wave A:** ADR-0010 broker + manifest + grant-aware harness | planned | **P0** | New `src/stdlib/broker.c` (fs.read/fs.write/net/env.write/process.spawn); `[capabilities]` in `tkc.toml` (`config.c`) + `--allow-*` (`main.c`) + baked into `.tkc.md` (`companion.c`); structured missing-grant diagnostic (RTcap). Build first-party manifests (ooke/website/self-tests) + wire grants into the corpus/eval harness. Also the concrete AMB-02..08 / COM-01/02 defect fixes (121.15/.22, 122.10). |
...
| 124.6 | **Wave C:** flip the two breaking defaults (bundled) | planned | **P0** | Owner: flip immediately on implementation. Bounds-by-default ON **+ flip user-program default emission to `-O2`** (`main.c:355`) **+ `@nobounds` annotation** (reuses `@wrapping`/`@checked` grammar — no new keyword). Deny-by-default ON (harness grant-aware from 124.4). **Measure at flip:** Epic 118 runtime residual + held-out Pass@1. |
