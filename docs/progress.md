...
| 121.1b | Migrate clang/linker shell-outs to argv-exec | planned | P1 | Follow-up to 121.1. Replace `system(cmd)` in `src/llvm.c` (clang invocation) with `posix_spawnp`/`fork`+`execvp`, one argv element per flag/path, so no shell is involved. The interim 121.1 fix rejects shell metacharacters in user-controlled paths (--out/--target/IR path); argv-exec removes the shell entirely and is the clean end-state. Care: `sources`/`all_libs`/`tls_flags`/`vi` are space-separated multi-token strings that must tokenize into argv elements. |
