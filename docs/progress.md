...
| 114.55 | ABI: a user `T!$err` function whose ok value is integer 0 / f64 0.0 is misread as an error (0-sentinel collision) | planned | **P1** | The error-union ABI returns the ok value or a `0` sentinel for the error, so a *user* function like `f=search(o):u64!$err{ … <count }` returning `count==0` is taken as `$err`. 114.53/114.54 fixed the **stdlib** number-parse wrappers by signalling via `tk_current_error`; the general fix is to route **all** `T!$err` ok/err through `tk_current_error` (a thread-local "did-error" flag set on the `<$err{…}` path, cleared on ok return) and have every error-union match/propagation decode on it instead of comparing the value to 0. Touches the `<` return, the 2-arm match, and `!` propagation. Until then, design fallible functions to return a struct/`$str`/non-zero ok type, or check preconditions before calling (as the shipped `tkgrep`/`datapipe` do). |
