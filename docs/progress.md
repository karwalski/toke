...
| 102.24 | Runtime crash: s.format() segfaults | planned | | `s.format(3.14;"%.2f")` crashes. Used by 3/7 segfaulting programs. The function builds but dereferences something wrong at runtime. Check str_glue.c for format implementation — likely ABI mismatch passing float as i64 to snprintf. |
| 102.25 | Runtime crash: array out-of-bounds returns garbage → null deref | planned | | `arr.get(N)` where N >= arr.len returns garbage pointer. When passed to `s.tofloat()` or `s.toint()`, causes segfault. Fix: bounds check in array access glue, return 0 or empty string for out-of-bounds. Affects 4/7 segfaulting programs. |
...
| 102.28 | Compile fail: E4070 mutability — model uses immutable where mut needed | planned | | 3/5 compile failures are E4070 "cannot assign to immutable binding". Model writes `let x=0; x=x+1` instead of `let x=mut.0; x=x+1`. Repair prompt needs stronger mut. instruction. Not a compiler bug. |
