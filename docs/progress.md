...
| 116.4 | **A4** — eliminate parser backtracking + multi-token lookahead | **loop-init DONE (via A3); `.get` pending** | P1 | The unbounded loop-init scan is gone (116.3). **Remaining (A4-get):** rework the `.get(...)` postfix production (`parser.c:664-679,724-736`) to parse the arg list once and branch on `arg_count==1` (single ⇒ `NODE_INDEX_EXPR`; else field+call) — no `p->pos=save` backtracking; and remove the residual 3-token type/closure peeks (`409,526`). Independent of A3. **A4a research:** whether `[]` indexing is the only clean fix (vs the `@()` decision). |
