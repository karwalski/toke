...
| ID | Story | Status | Priority | Notes |
|----|-------|--------|----------|-------|
| 124.0 | **Wave 0 (BLOCKING):** root-of-trust codegen fixes (umbrella → 124.0a/b) | planned | **P0** | A compiler that emits wrong values cannot be allowed to generate corpus. **Gates all codegen-adjacent work (124.2, 124.6, ADR-0014).** Decomposed into 124.0a/124.0b. |
...
| 124.0b | Differential-test harness over the Epic 118 corpus | planned | **P0** | Diff emitted-program behaviour (exit-code/output) against a known-good baseline across the ~2,100-program corpus; wire into CI. The safety net that must be green before **any** `src/llvm.c` change (124.2/124.6/ADR-0014) may influence corpus generation. Distinct from 123.6 (which adds unit cases); this is the diff-vs-baseline regression gate. |
