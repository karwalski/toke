...
**RESEARCH / PLANNING — do not implement yet; a detailed proposal is being produced in plan mode.** Prompted by the honest program-library results (114.27 follow-up): the corpus is real but **not written efficiently** — pervasive multi-line "one mutation per line" style, verbose boilerplate, and some runtime-inefficient solutions, which both inflate token counts and misrepresent idiomatic toke. Before training a model **from scratch**, the foundations must be locked: a *true* LL(1) language built for LLM generation + reasoning, a canonical minimal-code idiom, high-quality training data, and a clear story for agentic tool use (Claude Code, Cursor, Codex). This epic audits those foundations and proposes the structural/language/compiler/data changes + open research decisions required to realise the token-efficiency and reasoning benefits.

| ID | Story | Status | Prio | Notes |
|----|-------|--------|------|-------|
| 116.1 | LL(1) grammar audit vs LLM-generation goals | planned (research) | P1 | Confirm the grammar is *actually* LL(1) (single-token lookahead, no backtracking/ambiguity); catalogue where it holds/breaks and what LL(1) buys an LLM (deterministic incremental parse, cheap validity checking, constrained/grammar-guided decoding, precise error recovery). Recommend what to preserve/strengthen. |
| 116.2 | Canonical idiom + minimal-code best-practice standard | planned (research) | P1 | Define the *one right way* to write each construct for minimum tokens on first shot: expression-oriented over statement-per-line, batched/compound mutation vs one-mutation-per-line, chaining, avoid boilerplate, prefer stdlib. Feeds a formatter/canonicaliser + linter. This is the biggest lever on token count and correctness. |
| 116.3 | Runtime-efficiency best-practice patterns | planned (research) | P2 | The library also surfaced runtime-inefficient solutions (e.g. O(N²) array building pre-114.18, manual parsing where stdlib exists). Define performance idioms + guidance so "idiomatic" also means "fast". |
| 116.4 | Training corpus quality gate (regenerate to idiomatic/efficient) | planned (research) | P1 | Before from-scratch training, decide how to raise corpus quality: re-generate/repair to the 116.2/116.3 standard, filter, or hold-and-curate. The current corpus overstates verbosity; a model trained on it learns the verbose style. Define the quality bar + pipeline. |
| 116.5 | Reasoning integration for a toke-native model | planned (research) | P1 | How should a reasoning model think in/around toke: reasoning in NL vs toke, companion files as reasoning/spec scaffold, chain-of-thought token budget, compile/run-in-the-loop (RLVR) shaping toward minimal correct code. |
| 116.6 | Companion-file role in training & inference | planned (research) | P2 | toke has no comment syntax — documentation/intent lives in companion files. Spec how the model consumes/produces companions (code↔doc pairing, when to emit, format) so intent isn't lost while code stays token-minimal. Cross-ref [[project_serialization_vision]]. |
| 116.7 | Agentic tool-use & token efficiency (Claude Code / Cursor / Codex) | planned (research) | P1 | How the token savings are actually realised in agent loops: minimal-diff editing, grammar-constrained decoding, MCP/tool-call formats, context packing, `.tki` interfaces as compact API context. What toke/tooling must provide to win in these harnesses. |
| 116.8 | Structural / compiler / language changes required | planned (research) | P1 | Consolidate the concrete foundation changes the above imply (e.g. canonical formatter + deterministic minified form, grammar-constrained-decoding artefacts, AST canonicalisation, tokenizer finalisation, any language ergonomics that block minimal-first-shot code) and the sequencing/decisions to lock **before** training from scratch. |
