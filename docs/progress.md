...
| ID | Story | Status | Date | Notes |
|----|-------|--------|------|-------|
| 99.14 | Compiler bug: W1020 fires on $void which is a valid type | planned | | Lexer foreign-keyword detection triggers on bare `void` even when preceded by `$` (type sigil context). `$void` is a legitimate return type per spec Section 8.2. Fix: suppress W1020 when the token is preceded by `$`. Affects generated code quality — workers see a warning on valid programs. |
...
| 102.10 | False warning audit and compiler fix | planned | | Analyse W1020 and other warnings from worker runs. Identify false positives (like $void). Fix compiler. Ensure warnings in training data are genuine. |
| 102.11 | 4.4.1 Language improvement proposals from failure analysis | planned | | Analyse all worker failures: missing stdlib functions, syntax patterns that fail, unhelpful error messages. Propose concrete changes to language/stdlib/compiler. |
...
| 102.16 | Audit ALL .tki interface files vs C implementations | planned | | io.readln was missing — what else is? Scan every stdlib .tki for declared functions and verify each has a corresponding C implementation in *_glue.c or *.c. Report all gaps. Priority: functions used in the training corpus and worker-generated programs. |
| 102.17 | Rerun Epic 100 workers with fixed stdlib | planned | | Rebuild tkc on workers with io.readln fix. Rerun the 1700+ generated programs through the test harness. Programs that already compile may now PASS functional tests. Count actual working programs before/after. |
...
| 102.19 | Build chain reliability: 99.999% compile-to-binary success | planned | | Current build chain fails unpredictably: (1) Worker script used `tkc -o` (invalid flag, should be `--out`), (2) `tkc --out` on Linux fails with missing -lsqlite3, (3) Manual `--emit-llvm` + clang requires knowing ALL stdlib .c files to link, (4) Different subsets of stdlib needed per program based on imports. Fix: implement proper dependency resolution — tkc reads imports from source, resolves transitive stdlib deps from .tki, outputs the exact list of C files needed for linking. `tkc --emit-deps prog.tk` outputs the clang command. Or: `tkc --build prog.tk` handles the full pipeline internally (emit IR + invoke clang with correct deps). Target: ANY valid toke program builds to binary in ONE command with ZERO manual dependency management. |
...
| 102.21 | Compiler bug: lp() with < in condition parsed as return | planned | | `lp(idx<n)` fails — parser sees `<n` as return statement, not less-than comparison. Affects while-loop form `lp(condition){body}`. The C-style `lp(init;cond;step)` works fine. Either fix the parser to distinguish `<` in loop condition context, or document that while-loops require `lp(let cond=true;cond;){...}` pattern. Extremely common in generated code. |
| 102.22 | Compiler bug: LLVM IR codegen type mismatch (i8* vs i64*) | in_progress | 2026-05-26 | v0.3.3 fixed getelementptr but NOT store instructions. 4/25 worker failures are this bug. Pattern: `store i8* %tN, i64** %var` and `store i64* %tN, i8** %var`. Need to add bitcast before ALL store instructions involving pointer types, not just GEP. |
| 102.23 | Missing C glue: str.charcode, math.ln, float.parse, fmt.f64, array.get | planned | | 5/25 worker failures are undefined symbols. These functions are used by generated programs but have no C implementation. Add to appropriate *_glue.c files. Part of the ongoing .tki/C gap (102.16). |
