...
| 124.4b | Compile-time grant baking: `[capabilities]` parse + `--allow-*` at compile + bake globals + `.tkc.md` | planned (next) | **P0** | Extend `config.c` with a `[capabilities]` TOML table; parse compile-time `--allow-*` in `main.c`; emit strong `@__tk_cap_baked_{grants,present,enforce}` globals (max_iters-style) overriding the weak defaults; write the `.tkc.md` companion recording the grant set. |
