...
| 118.7 | `bench/programs/` v0.4 residue | **in progress** (E2002 fixed; E1003 `[]` remaining) | P2 | **New 2026-07-06 (found via a stale `test-companion` fixture during 124.4-fu).** The compile-speed benchmark programs in `bench/programs/*.tk` were never migrated to v0.4. **Fixed (E2002 `=`→`==`):** `prime_sieve.tk` (unblocks `make test-companion`), `deep_recursion.tk`, `collatz.tk`, `gcd_euler.tk` — now `--check`-clean. **Remaining (E1003):** `binary_search.tk`, `sum_array.tk` use the removed `[i64]` array-type syntax → migrate to v0.4 `@(…)` (needs array-literal + index-site migration, not just the type annotation). |
...
| 124.4 | **Wave A:** ADR-0010 capability subsystem (umbrella → 124.4a–g) | **in progress** (enforcement machinery COMPLETE: a/b/c/c2/d + spec doc `docs/spec/capabilities.md`; all sinks gated, end-to-end enforced; default still ALLOW_ALL until 124.4g. Remaining: e/f/g flip-readiness cluster) | **P0** | Deny-by-default capabilities; grants declared out-of-band so the source grammar is untouched. **Threading mechanism DECIDED 2026-07-06 (owner-confirmed):** option (a) — the compiler bakes the required grant set into the binary + the program accepts runtime `--allow-*` flags + a runtime broker gates each sink; effective grants = baked ∪ runtime. Chosen over (b) runtime `tkc.toml` read (binary not self-describing/portable) and (c) env var (ambient/spoofable). Precedent: the compiler already bakes `max_iters` inline (`llvm.c`), and `tk_runtime_init(argc,argv)` is the guaranteed startup hook. Decomposed into 124.4a–g. |
...
| 124.4e | First-party capability manifests | planned | **P0** | ooke, the website, and stdlib self-tests declare their real footprint (webroot read, listen socket, cert/key paths, DB socket, any `process.spawn`) via `tkc.toml [capabilities]`. Validates the broker before the flip. Depends on 124.4c/c2. |
| 124.4f | Grant-aware corpus/eval harness | planned | **P0** | Per-program manifests or a `--allow-all` test mode in the corpus/eval harness so I/O tasks aren't failed closed. **Hard prerequisite for the deny flip** — without it every I/O task's Pass@1 → ~0. Reaches into the `toke-model/corpus` harness. |
| 124.4g | Flip deny-by-default (+ argv-strip + ambient defect fixes) | planned | **P0** | The flip: change the broker default to `TK_CAP_MODE_ENFORCE` (currently ALLOW_ALL). **Only after 124.4c2/e/f land.** Also: **strip consumed `--allow-*` from the program's argv** before `tk_str_argv` sees them (Deno-style; today the program still sees them — noted during 124.4c). Folds the ambient/toolchain defect fixes (121.15/121.22 + 122.10: argv-vector exec default, `lstat` `rmdir_r`, containment `path_join`, `O_NOFOLLOW`, dotenv key allowlist, PATH snapshot) and routing the compiler's own child-`clang` through a grant (120.2 COM-01/02). |
...
| 124.6d | Flip capabilities to deny-by-default | planned | **P0** | Turn the broker default to deny. **Hard-gated on 124.4f** (grant-aware harness) + 124.4e (first-party manifests) so training/eval Pass@1 isn't destroyed. |
