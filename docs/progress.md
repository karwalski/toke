...
| 124.4i | `test_tk_runtime` pre-existing crash | planned | P2 | **New 2026-07-07 (found re-linking `test-stdlib-runtime` during 124.4h).** The target was link-broken since 124.4a (undefined `tk_cap_init`); once relinked it **SIGTRAPs** partway through `test_tk_runtime.c` — **after** the argv + all `tk_json_parse` (int/bool/str/array) cases pass, in a later test. Confirmed **not** from 124.4h (a minimal harness calling `tk_runtime_init`/`tk_str_argv`/`tk_path_snapshot` passes). Bisect the post-array tests in `test_tk_runtime.c` to find the trapping case (likely a JSON-object or nested path). |
