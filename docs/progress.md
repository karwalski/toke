...
| 124.3 | **Wave A:** ADR-0011 runtime parts | planned | **P0** | Delete `spawn_shell` wrappers + argv-only guarantee (`process_glue.c:18`); fix the MySQL param-bind bug (DAT-03); build the **context-aware auto-escaper** (BODY/ATTR/URL/JS/CSS) in `template.c` (`:875`) + point `tpl.render`/ooke `{=x=}` at escape-on; stop `CMARK_OPT_UNSAFE`. Sweep the live ooke site. |
