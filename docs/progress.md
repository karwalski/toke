...
| ID | Story | Status | Date | Notes |
|----|-------|--------|------|-------|
| 112.1 | Semantic bug: `let X=io.readln()` binds LAZILY — multi-read collapses to last value | planned | 2026-06-01 | **P0** Reproducer: `m=t;i=io:std.io;f=main():$i64{let a=io.readln();let b=io.readln();let c=io.readln();io.println(a);io.println(b);io.println(c);<0}` with `printf 'L1\nL2\nL3' \| prog` prints `L3\nL3\nL3`. Wrapping the call forces eager evaluation: `let a=s.trim(io.readln())` correctly yields `L1\nL2\nL3`. Likely cause: `let X=side_effect_call()` defers evaluation to first use of `X` instead of binding the value at let-time, so all three reads happen at print-time and stdin returns the same residual value. **Impact:** 5/5 net-rest failures studied had this bug; likely affects many more. **Suggested fix:** `let` MUST eager-evaluate the RHS at binding time (esp. when RHS is an `io.*` or `mut.*` call). If laziness is intentional, mark it with `lazy.` syntax and keep `let` strict. See [[reference-toke-lang-gotchas]]. |
| 112.2 | Semantic bug: string `=` equality asymmetric — `if(a=b)` returns false for identical vars | planned | 2026-06-01 | **P1** Reproducer: `let a=s.trim(io.readln());let b=s.trim(io.readln());if(a=b){io.println("EQ")}el{io.println("NEQ")}` with `printf 'hello\nhello' \| prog` → prints `NEQ`. But `if(a="hello")` with same input → prints `EQ`. So `=` works as string comparison when ONE side is a literal but NOT when BOTH sides are variables. Workaround: `s.equals(a;b)` works for var-to-var. **Likely cause:** literal side gets interned/compared by content; variable comparison falls through to pointer-identity (different heap addresses for equal-content strings). **Suggested fix:** make `=` always content-compare for `$str`, matching int-equality semantics. See [[reference-toke-lang-gotchas]]. |
| 112.3 | Runtime crash: array `.set(i;v)` SIGSEGVs on mutable arrays | planned | 2026-06-01 | **P1** Reproducer: `m=t;i=io:std.io;f=main():$i64{let a=mut.@();a=a+@(10);a=a+@(20);a=a.set(0;99);io.println("never reached");<0}` → exit 139, no output. Compiles clean. Workaround: rebuild the array via lp + concat, e.g., `let out=mut.@();lp(let i=0;i<a.len();i=i+1){if(i=target){out=out+@(newval)}el{out=out+@(a.get(i))}}`. **Impact:** affects any program doing in-place array mutation (counters, sort, fixed-point updates). NET-197 uses `.set` and PASSES, so the bug may be type-specific or layout-specific. **Suggested fix:** investigate `.set` codegen for `@($i64)` and `@($str)` separately; ensure mutable array storage is correctly tracked through `a=mut.@()` declaration. See [[reference-toke-lang-gotchas]]. |
| 112.4 | Parser bug: `mut.s.method(...)` chain fails with "expected ')', got '.'" | planned | 2026-06-01 | **P2** Reproducer: `let mn=mut.s.tofloat(io.readln())` → parse error E2002. Workaround: two lets — `let firstVal=s.tofloat(io.readln()); let mn=mut.firstVal;`. **Likely cause:** `mut.` parser greedily reads next token as the bound-to identifier, doesn't handle a method-call expression. **Suggested fix:** allow `mut.<expr>` where `<expr>` is any expression (function call, method chain, literal), not just an identifier. See [[reference-toke-lang-gotchas]]. |
...
| 112.6 | Add canonical-pattern rules to TOKE_SYSTEM_PROMPT for 112.1–112.4 | planned | 2026-06-01 | **P0** Once the four bugs above are characterised (and ideally fixed in compiler), update `worker-generate.py:TOKE_SYSTEM_PROMPT` with explicit RULES: (a) "ALWAYS wrap `io.readln()` in `s.trim(...)` when storing in a `let` — never `let x=io.readln()`" (until 112.1 fixed); (b) "Use `s.equals(a;b)` for variable-to-variable string equality — `a=b` is unreliable unless one side is a literal" (until 112.2 fixed); (c) "AVOID `array.set(i;v)` — it crashes; rebuild the array via lp + concat" (until 112.3 fixed); (d) "Don't chain `mut.s.method()` — split into two lets" (until 112.4 fixed). Add WORKING examples for each. |
| 112.7 | Investigation: how many corpus failures are explained by 112.1–112.4? | planned | 2026-06-01 | **P1** Sweep all REGRESSED-RUN and REGRESSED-COMPILE programs in `toke-test-programs/results/solutions/` looking for the four patterns. Categorise each fail by which bug(s) it hits. Expectation: a meaningful fraction of the current ~1700 compile-fails + ~240 run-fails are downstream of these four issues. Output: a count per bug, a list of "fixable by pattern" programs, and an estimate of pass-rate uplift if 112.1–112.4 are addressed at compiler level. |
| 112.8 | Hand-repair backlog: continue manual repairs of remaining data-processing eligible failures | planned | 2026-06-01 | **P3** 10/20 attempted data-processing repairs passed (DAT-001/002/004/007/009/010/011/012/013/032). Remaining 10 targets stalled on compiler/lang issues or algorithmic complexity beyond hand-rewriting: DAT-005 (JSON Schema), DAT-006 (YAML to JSON), DAT-008 (TOML), DAT-014 (Pivot), DAT-015 (Z-Score, needs sqrt), DAT-016 (Min-Max — chained mut bug 112.4), DAT-082 (Funnel multi-line), DAT-089 (Levenshtein/Jaro-Winkler), DAT-112 (Pipeline), DAT-114 (Time zone — impossible without TZ DB). Revisit once 112.1–112.4 are fixed; expect several to become repairable. |
