...
| 114.18 | **OPEN (deferred — architectural)** | Copy-on-write arrays / O(N²) in-place mutation is a runtime redesign, not a point fix. Left open. |
| 114.19 | **PARTIAL** | 19a (`byte`/`bit` local) and 19b (alias-shadow) no longer segfault. 19c (`@(a)+(b)` — appending an **unwrapped** scalar) still exit-139s: making it a compile-time diagnostic needs concrete array-literal type inference (untyped `@(a)` infers TY_UNKNOWN and skips the operand check) — deferred as invasive vs the user-error value. |
...
| 114.21 | **DONE (type) — runtime gated on 114.4** | The E4031 "won't concat `@(int)` with `@byte`" is fixed by the 114.10 array-concat typing. Runtime byte-array `+` concat still has a layout bug (`@(72;105)+@(33)` renders garbage while non-concat `@(72;105;33)` is correct) — that's the 114.4 bytes-ABI, tracked there. |
| 114.22 | **OPEN (deferred — cosmetic P3)** | `crypto.tki`/`encrypt.tki` underscore names vs the no-underscore callable spelling. Misleading-only (the compiler strips underscores when matching); a blanket `.tki` regen risks the checker's matching, so left for a deliberate interface pass. |

Net: **114.10/11/12/15/20/21 fixed**, **114.13/14/16 closed-not-reproducing**, **114.18/22 deferred** (architectural/cosmetic), **114.19 partial**. All alongside 114.17 (encrypt) and 114.25/114.26 earlier this day. `make conform` 180/0 throughout.
