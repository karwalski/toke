...
| 113.B.10 | Spec/tooling: decide whether `.tki` exports must faithfully encode `!error` unions (currently erased) and update interface-gen + checker | planned | P2 | all (interface fidelity) | e.g. `configload` exported as `ookecfg`, source is `$ookecfg!$configerr`. **Confirmed empirically (tier-0):** `.tki` also collapses map/array field+return types to a bare `@`, losing key/value/element types (`$routematch.params:@($str:$str)` → `"@"`). |
...
|----|-------|--------|------|-------|
| 114.1 | Codegen: LOCAL/returned `@$f64` arrays + non-literal float appends surface elements as i64 | planned | **P0** | **Follow-up to 110.9** (which fixed only the *parameter* path — that's why the agent workaround "pass the array into a typed-`@($f64)` param helper" works). LOCAL build, function-RETURN, and appending a **non-literal** float still revert the element to i64: `arr.get(i)` prints right via `s.format` but uses integer add/mul → `RT002: integer overflow` / garbage. Seeding `mut.@(0.0)` survives only LITERAL appends; `r=r+@(varFloat)` reverts to i64 with **no source workaround** → hard-blocks dynamic float matrices (MFG-022). **8 agents.** Repro: `f=mk():@$f64{let r=mut.@(0.0);r=r+@(s.tofloat("3"));<r}` then `mk().get(1)*mk().get(1)` → overflow. |
...
| 114.3 | Codegen: `array.set(i;v)` is a silent no-op (write never lands) | planned | **P1** | Compiles+builds clean but `let a=mut.@(1;2;3);a.set(1;9);a.get(1)`→`2`. (112.3 fixed the `.set` SIGSEGV; mutation-lost is separate/regressed.) **5 agents.** Forces O(n²) rebuild-by-concat in every sort/update. |
| 114.4 | Stdlib/ABI: `[byte]` arrays broken — crypto↔encoding incompatible, digests are hex-text not raw bytes | planned | **P1** | `crypto.sha256`/`hmacsha256` return the 64-char hex **text** as bytes (not 32 raw bytes); `crypto.sha256(s.bytes(x))` ≠ `crypto.sha256(encoding.hexdecode(hex(x)))` for identical logical bytes; `[byte]` `.len`/`.get` garbage; `[byte]+[byte]` doesn't concat; `crypto.hmacsha256` also fails RFC 4231. **5 agents.** Hard-blocks PBKDF2/scrypt/HMAC/X3DH (no way to feed raw digests into a KDF chain). |
...
| 114.8 | Stdlib capability gaps (features, not bugs) — bitwise ops + missing primitives | planned | P2 | No bitwise operators (`^ & << >>`, "deferred to v0.5") → blocks AES/ChaCha/SHA3/MD5/RIPEMD natively (some agents emulated XOR/GF via `/`+`%`; CRY-021 AES-256-CBC fully passed that way). Missing stdlib: MD5, SHA3/Keccak, RIPEMD-160, NIST P-256, scrypt, bcrypt; `os.stat` exposes size only (no perms/owner/group). Tracking story — decide which to implement vs document-as-unsupported. |
| 114.9 | Corpus: ~25 test fixtures contradict their own verified Python reference | planned | P2 | Cleanup (corpus, not compiler). Agents found ~25 programs whose `expected_output` disagrees with the verified Python ref output: wrong hash constants, wrong arithmetic (`0.500001`, `304.33` vs ref `304.23`), literal placeholders (`N`, `LOCKED_BY (pid)`), impossible column widths, RNG/Mersenne-Twister-dependent outputs. Agents correctly refused to hardcode these, so true repair rate > strict-fixture pass count. Examples: CRY-001, GAM-034, GAM-053, SYS-057, SCI-038, FIN-013/027/050, MED-008, MSG-015. Regenerate fixtures from the authoritative reference or quarantine the RNG-dependent ones. |
