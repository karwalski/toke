...
| 124.2d | Nil-deref trap (RT005) — parked | parked | P1 | Split from 124.2. Guarding struct-field GEP against a null base is broad (≈23 field-GEP emit sites in `llvm.c`) and risks per-access overhead + breaking valid null-base-never-derefed patterns. Needs a focused design: which sites to guard, the perf tradeoff, and how to prove-non-null where possible. Do as a dedicated attended increment, not an autonomous loop tick. |
