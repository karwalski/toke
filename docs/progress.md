...
| 114.56 | Codegen: string comparison `<`/`<=`/`>`/`>=` on a **function-returned** string falls back to pointer comparison instead of `strcmp` | planned | P2 | **Found 2026-06-30 while implementing the MSG-034 toke solution (114.27).** 114.2 added strcmp-based string `<`/`<=`/`>`/`>=`, but the operator only lowers to `tk_str_lt_w`/etc. when codegen statically sees both operands as strings (`i8*`). A string **returned from a function** crosses the ABI as `i64` and its static type erases, so the comparison emits an integer `icmp` on the two **pointer addresses** instead of a content compare — the result depends on allocation order, not characters. **Repro:** `f=ret(x:str):str{<x}; … let a=ret("zzz"); let b=ret("aaa"); a<=b` → `true` (wrong; `a>b`). Literals and let-bound-from-literal strings compare correctly; only erased-type (function-returned, and likely other i64-typed) string operands are affected. Same erasure family as 113.B.10/114.16. **Workaround in the wild:** MSG-034's `solution.tk` compares its two 30-digit strings with a manual char-code helper (`s.charcode`) instead of `<=`. **Fix:** make the comparison operators consult the richer string-typedness signal (e.g. treat an operand whose source is a `str`-returning call as a string), or have string-returning calls preserve `i8*` typing, so `<`/`<=`/`>`/`>=` route to the strcmp wrappers. |
