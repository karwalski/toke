...
| 80.2.7 | Match on strings with $variant patterns generates ok/err bifurcation | backlog | — | **P0** `mt tier {$interactive:v ...;$considered:v ...;$background:v ...}` where `tier:str` generates null-check (ok/err) instead of string comparisons. Only first arm ever executes. Affects test_tiers, tierlabel, priorityrank. Needs: match codegen to detect string-typed expr and emit `strcmp` chain for `$variant` arms. |
