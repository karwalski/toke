...
| 126.5 | **Codegen: nested int-array element mis-typed `TY_STR` → strcmp crash.** `expr_llvm_type` trusted a spurious `TY_STR` on a nested subscript (`dist.get(i).get(j)`), so `dist.get(i).get(j) >= 999999` lowered to `strcmp` on a non-pointer i64 → SIGSEGV. **PARTIAL DONE 2026-07-08** (`7eb495e`): direct-subscript comparison now numeric (`test_nested_arr_compare.tk`; conform 183/0; diff-codegen 0). **SCI-019 now PASSES** (fixed by the compiler work — the nested-index compare guard + string-array typing); **5 corpus crashers remain** (SCI-082, MSG-079, AIA-091/124, SEC-071) — a *deeper*, hard-to-minimise SIGSEGV: nested-array rebuild + full read; every reduction runs, only the full programs crash. Needs dedicated IR-level bisection. | in progress | **P0** |
| 126.6 | **Codegen: `$str` struct-field access typed as `i64` → prints pointer-as-decimal.** `expr_struct_type` had no field-access case, so `\(rec.name)` missed the interp `is_str` gate. **DONE 2026-07-08** (`e72eaba`): plain struct-field `$str` now typed correctly (verified `\(rec.name)`→"alice"; conform 183/0; diff-codegen 0). **Residual:** the 6 WRONG_OUTPUT (AIA-102/097/100/105/044, DEV-109) use `reqs.get(i).raw` on a **json-array element** built via **stale json API** (`json.getobj`/`getarr`, not in the current `.tki`) → the `[Json]` element type isn't tracked, so `.raw` still renders a pointer. Needs array-of-struct element tagging + json-API-drift resolution (→ 126.8). | in progress | P1 |
...
| 126.8 | **Residual failures.** (a) 6 json-`.raw` WRONG_OUTPUT (126.6 residual) — need array-of-struct element type tagging + migrate stale `json.getobj/getarr` to the current `json.arr` API. (b) 6 deep crashers (126.5). (c) 3 FILE_DEP (DEV-049/SYS-081/SYS-125) — read input files the manifest test_cases don't provide; the programs are likely correct but untestable without fixtures (provide files in the harness, or tag fixture-dependent). | planned | P2 |
