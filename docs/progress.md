...
| 114.44 | Language: no module-level mutable global state | planned | **P1** | toke top-level allows only `NAME = literal : type;` constants — `let store=mut.$bookmarks{…}` (or even `let g=mut.0`) at module scope is `E2002`. A stateful server (the bookmarks-api CRUD demo keeps an in-memory `$bookmarks` store mutated across request handlers) has nowhere to hold state — handlers are invoked by the http framework with no way to thread it through. Fix = a mutable-global declaration form + codegen emitting an LLVM global, running heap-allocating initializers from a startup hook before `main`, with name resolution reading/writing the global. Blocks **bookmarks-api** and any stateful server/CLI. |
| 114.43 | Stdlib: `std.csv` — `csv.parse` fixed; streaming `reader`/`next`/`header` still unimplemented | **PARTIAL** (4f3ac37) | **P1** | `csv.parse(s.bytes("a,b\n1,2"))` returns an empty `[csvrow]` (0 rows) for valid CSV. The streaming API declared in `csv.tki` (`csv.reader`/`csv.next`/`csv.header`/`csv.writer`/`csv.writerow`/`csv.flush`) has **no `_w` wrappers** at all — only `tk_csv_parse_w`/`tk_csv_serialize_w` exist, so any program using the streaming API fails to link (`Undefined: _tk_csv_reader_w`). Blocks **datapipe** (CSV→stats→JSON) and any CSV program. Fix: repair `csv.c`/`csv_glue.c` parsing and implement the streaming wrappers (or remove the unbacked `.tki` entries). |
