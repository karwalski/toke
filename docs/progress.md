...
| 126.8 | **Residual failures — 8/1460 (library at 99.5% PASS).** (a) **5 FILE_DEP** — SEC-071/DEV-049/SYS-081/SYS-125/DEV-109 read input files the manifest `test_cases` don't provide (programs likely correct; also `f.read` on a missing file SIGSEGVs — a robustness gap worth a graceful-error fix). Provide fixtures in the harness or tag fixture-dependent. (b) **3 WRONG** — AIA-044/100/105 (json/logic; AIA-100/105 pre-existing, not the 126.6 field-typing bug). | planned | P2 |
