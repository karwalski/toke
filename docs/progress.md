...
| 113.3 | Rebuild `ooke.store` (flat-file content store, frontmatter) + companion + tests | review | feature/pure-toke-rebuild | Rebuilt: canonical `$ok`/`$err`, companion, test green. **Carries workarounds — NOT parity-complete:** `storesqlcreate` emits base columns only (blocked 113.B.15 map.keys), `storefind` returns not-found error directly (blocked 113.B.12 struct-field-map read). Faithful version needs 113.B.12 + 113.B.15. |
| 113.4 | Rebuild `ooke.router` (file-system route scan + match, dynamic segments) + companion + tests | review | feature/pure-toke-rebuild | Rebuilt: canonical, companion, test green. **Workaround:** static-segment equality uses `str.len`+`str.indexof` instead of `=` (blocked 113.B.11 string identity-compare). Faithful version needs 113.B.11. |
| 113.5 | Rebuild `ooke.template` (lexer/parser/renderer, layout, partials, islands, filters incl. `md`) + companion + tests | review | feature/pure-toke-rebuild | Rebuilt: canonical, companion, test green. **Workarounds:** typed empty map literal for cache init (113.B.14), test restructured to avoid match-bound-struct nested-map re-read (113.B.12). Single file retained (multi-file-per-module not supported by the per-module .tki/IR pipeline). |
| 113.6 | Rebuild `ooke.build` (static gen, CSS inline, minify, asset copy) + companion + tests | planned | — | tier-2; depends on 113.2/3/4/5 + validate. |
| 113.7 | Rebuild `ooke.serve` (HTTP server, workers, dynamic GET/POST/PUT/DELETE/PATCH handlers, CORS, api-prefix, TLS, access/error logs) + companion + tests | planned | — | tier-2; depends on router/template/store. This is where most loke plumbing parity lives. |
| 113.8 | Rebuild `ooke.cli` (`main`, `new`, `gen type/page/api/island`, `repair`) + companion + tests | planned | — | tier-3; depends on all. `test_cli` currently fails harness (ooke.cli excluded from module-compile). |
| 113.9 | Rebuild remaining modules (`apihealth`, `validate`, `repair`, `run`, handlers) + companions + tests | review | feature/pure-toke-rebuild | **`apihealth`/`validate`/`repair` DONE (clean — only `$ok`/`$err` migration, no workarounds, tests green).** `run` rebuilt but carries workarounds (blocked 113.B.13 `!err`-in-`if`, 113.B.6 process.spawn argv). `handlers` not yet rebuilt (tier-1). |
| 113.10a | Revert tier-0 workarounds after P0 compiler fixes — restore clean, efficient, parity-faithful code | planned | — | **Once 113.B.11/.12/.13/.14/.15/.6 land, revisit every workaround and revert to the clean implementation, then re-verify tests.** Explicit checklist: (a) `store.storesqlcreate` — restore per-field column generation via `map.keys` (113.B.15); (b) `store.storefind` — restore the loop reading `item.meta.get(key)` (113.B.12); (c) `store.storeslug` + any `item.slug=slug` equality — confirm content-equality once 113.B.11 lands; (d) `router.routertrymatch` — restore `pseg=useg` static-segment equality, drop the `str.len`+`str.indexof` workaround (113.B.11); (e) `template.tplcachenew` — confirm typed-empty-map literal is the clean canonical form (113.B.14) and restore any test paths restructured around 113.B.12; (f) `run.runcompile` — restore `!err`-in-`if` short-circuit (113.B.13) and clean `process.spawn` argv (113.B.6). Each revert must keep tests green and be the most efficient canonical form. Search aids: workarounds are tagged in each module's `.tkc.md` Notes and the 113.3/.4/.5/.9 rows. |
| 113.10 | Integration: full `make test` green + loke plumbing-parity validation against the baseline | planned | — | Gate for declaring the rebuild at parity. loke runs on `toke-ooke-pure`. Requires 113.10a complete (no workarounds remaining). |
