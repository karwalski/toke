...
| 124.2 | **Wave A:** ADR-0012 guards + bounds-check codegen | **in progress** (division RT004 + stack canary DONE; nil-deref + bounds remaining) | **P0** | After 124.0. **DONE 2026-07-04:** (b) **division/remainder-by-zero guard (RT004)** — `sdiv`/`srem` now branch to `tk_div_trap` (new runtime helper, `tk_runtime.c`) on a zero divisor instead of machine UB; verified `10/0`/`10%0` trap deterministically, `10/3`=3, `10%3`=1, conform 180/0, **diff-gate 0 changes**. (e) **real stack canary** — flipped the inert `stack-protector-buffer-size` hint to `sspstrong` on the `#0` attribute group (all functions). **Remaining:** (d) nil-deref trap RT005 on struct-field GEP; (a) bounds-check codegen RT003 on `NODE_INDEX_EXPR` load+store, converging `.set`/`$vec` semantics — each differential-gated. (Flip to default-on is 124.6.) Folds 123.4/121.31/121.36/122.3. |
