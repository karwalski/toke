...
| 124.7 | **Wave D:** spec close-out + ADR-0014 (option/non-null type) | **in progress** (memory-model §6 DONE) | P1 | **DONE 2026-07-04:** `memory-model.md` §6.6 (spatial/arithmetic safety — RT002/003/004/005 traps + stack canary) and §6.7 (injection resistance — argv-only exec, context-aware HTML/URL/markdown/template escaping) now formally document the shipped guards. **Remaining:** Update `memory-model.md` §6 (RT003/4/5), `runtime-abi.md` (3 injection guarantees + envelope), `known-limitations.md`. **ADR-0014** (option/non-null as a first-class type, reusing `T!$none`+`mt`) is **gated on 124.0/123.5** and measured against memorizability before any new sigil. ADRs 0010–0013 already flipped to Accepted. |
