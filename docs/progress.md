...
| 126.5 | **Codegen: nested-array mutation crash (SIGSEGV)** — 6 library programs (SCI-019/SCI-082/MSG-079/AIA-091/AIA-124/SEC-071) crash. Simple `@(@(1;2);@(3;4)).get(1).get(0)` works, so it's the **array-of-arrays with reassignment/in-place** pattern (Floyd-Warshall-style `dist=newdist`; interacts with the 114.18 in-place array optimisation + element-type erasure 113.B.10). Regressed after the 2026-06-26 verification (114.18 landed 2026-06-30). Needs a minimal repro + fix in `llvm.c` array codegen. | planned | **P0** |
| 126.6 | **Codegen: map string-keys render as pointer addresses** — AIA-102 emits `{4352596928:null,…}` instead of `{"sender":…}`; a map with string keys serialises the key pointer, not the string. Fix map-key rendering/iteration in codegen or json glue. | planned | P1 |
| 126.7 | **Codegen: E9003 `double defined but expected ptr`** — AIA-076 emits invalid IR (`%t491 double but expected ptr`), clang rejects it. A float value flows into a pointer slot. Minimal repro + fix. | planned | P1 |
| 126.8 | Remaining WRONG_OUTPUT (AIA-044/097/100/105, DEV-109) — triage logic vs codegen; + decide FILE_DEP (DEV-049/SYS-081/SYS-125) test-fixture policy (provide input files in the harness, or tag as fixture-dependent). | planned | P2 |
