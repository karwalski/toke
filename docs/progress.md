...
| 114.2 | Codegen: heap-string `=`/`!=`/`<`/`<=` on DERIVED strings compares by identity / isn't lexicographic | planned | **P1** | **Same family as 112.1/112.2/113.B.11/113.B.21** — string typing across stdlib/derived paths is systematically incomplete. For strings from `arr.get`/`s.slice`/function-returns/cross-iteration: `=` pointer-compares (false for equal content); `>=`/`<` not byte-ordered (`"6">="0"`→false). **8 agents.** Workaround: `s.eq` + charcode comparators. Fix: mark all `$str`-producing paths (incl. array-element & call-return strings) so the strcmp/lexicographic path fires; audit relational-op codegen for `$str`. |
...
| 114.5 | Stdlib: math gaps — `ln`/`erf`/`log10`/`log2`/`sin`/`cos` missing; `floor`/`ceil` return bit-pattern | planned | P2 | `math.ln`/`erf`/`log10`/`sin`/`cos` pass `tkc --check` (lenient `math.*` resolution) but are absent → silently resolve to 0 or fail at link (`_tk_math_erf_w` undefined); `math.floor`/`ceil` return the float bit-pattern as i64. `exp`/`sqrt`/`pow` work. **3 agents.** Workarounds: Newton/series for ln, Abramowitz-Stegun for erf, `(x+0.5) as i64` for floor. |
| 114.6 | Stdlib bugs: `str.charcode` ignores index; `str.join` segfaults; `s.gt` codegen-fails; `crypto.tohex` double-encodes | planned | P2 | `s.charcode(str;i)` always returns char-0's code (workaround `s.charcode(s.slice(str;i;i+1);0)`) — 2 agents. `str.join(sep;arr)` SIGSEGV (RC139) for any string array even literals — 1. `s.gt` → clang/codegen failure E9003 — 1. `crypto.tohex` returns hex-of-hex-text (use `encoding.hexencode`) — 2. |
| 114.7 | Frontend: `tkc --check` ≠ buildable; `lp(var=literal)` misparse; mut-shadow false E4070 | planned | P2 | **`--check` accepts hallucinated APIs** (`std.json` builders `j.object/array/push/stringify`, missing `math.*`) that then fail at link with undefined symbols — type-checker too lenient on unresolved `module.method`; tightening it would catch a whole class of bad generations at check-time (**3 agents**). `lp(go=1)` mis-parses as assignment → "expected ';'" (2 agents; use `lp(go>0)`). Shadowing an immutable same-name binding → spurious E4070; `mut.(callexpr)` doesn't bind mutably. Reserved/undocumented identifier traps: `sc`, `br`, `rt`, `fn`, `k`. |
...
| 114.18 | Runtime/codegen: arrays are copy-on-write with no in-place mutation → O(N²) for large-array algorithms | planned | **P1** | `arr.set(i;v)` and `arr+@(x)` copy the entire backing block each call; no amortized push / in-place store. ~300k `.set` on a 10k array ≈ 5s. **crypto-sweep agents 1 & 2.** Hard-blocks scrypt N=16384 (CRY-017, O(N²) BlockMix) and times out P-256 scalar-mul >10s (MSG-006) though both are algorithmically correct. Small-array crypto (AES/ChaCha/bignum) unaffected. Fix: in-place `set`/growable buffer or escape-analyzed mutation. |
| 114.19 | Codegen: three silent-segfault footguns (compile clean, exit 139, no stdout) | **PARTIAL** | **P1** | **crypto-sweep agent 2 (MSG-044).** (a) a local named `byte` or `bit` → codegen corruption/segfault (repro: `f=g():$i64{let byte=1;<byte}` in a loop); (b) shadowing an import alias with a local of the same name (`i=s:std.str;` then `let s=mut.@(…)`) → state corruption/segfault; (c) appending an *unwrapped* scalar to a byte array (`r=r+@(a)+(b)`, last term not `@(b)`) compiles but segfaults. All three should be compile-time diagnostics, not runtime crashes. Workarounds: rename locals; never reuse alias names; wrap every appended element `@(…)`. |
...
| 114.22 | Interface/docs: `crypto.tki` declares `crypto.to_hex` but the callable name is `crypto.tohex` (underscore rejected E1003) | planned | P3 | **crypto-sweep agents 1, 2, 4.** The `.tki` lists the underscore spelling, but no-underscore Profile-1 strips it, so `crypto.to_hex` is a source error and `crypto.tohex` is the live alias. Regenerate `crypto.tki` to the no-underscore spelling so the interface matches what compiles. Harmless but misleading. |
...
| 114.27 | Corpus-spec: MSG-034 contact-fingerprint fixtures are unverifiable placeholders (no real algorithm can produce them); also needs SHA-512 | planned | P2 | Disabled `[CHEAT-AUDIT]`. Expected outputs (`12345 67890 …`, `xxxxx xxxxx …`) are placeholders and the test input `aaaa...key_hex` is not valid hex — **no genuine safety-number/SHA-512 implementation can reproduce them**, so only the existing input-discriminator cheat passes. Define the concrete algorithm (hash rounds/encoding/group format) and regenerate fixtures from a verified reference. Note `std.crypto` exposes no SHA-512 (only sha256). |
...
| 114.32 | Codegen: string interpolation of a non-string value (`"\(n)"`, n:$i64/$f64/$bool) passes `--check` then segfaults at runtime | planned | **P2** | `let n=5; io.println("val=\(n)")` -> exit 139. Should be a check-time type error or an auto-conversion. Workaround: interpolate `\(s.fromint(n))` / cast `as $str`. r11 (AIA-112). |
...
| 114.36 | Codegen: `env.home`/`home` lowers to `use of undefined value '%home'` | planned | P2 | `about_ooke` / `about_web_server` doc examples emit invalid LLVM IR (`%home` referenced but never defined) — a NODE lowering drops the binding. `--check` passes; clang link of the `.ll` fails. |
| 114.37 | Codegen: f64 typed as `ptr` in an error-handling example (`'%t3' defined with type 'ptr' but expected 'double'`) | planned | P2 | `guide_05_errors` doc example: an f64 value crosses a path where it's tracked as a pointer, producing a type-mismatched `.ll`. Same family as the 114.5/114.16/114.30 f64-typing line. |
| 114.38 | Build fails on a W-code diagnostic in 3 stdlib/cookbook examples | planned | P3 | `cookbook_data_pipeline` (W1001), `guide_06_strings_io` (W1020), `guide_08_advanced` (W8001) build-fail with only a **warning**-class diagnostic surfaced — either warnings are fatal in `--out`, or a real error is masked behind the warning. Investigate. |
| 114.39 | Stdlib: no string→float parser (`str.tofloat`/`str.tof64`) | planned | P2 | `str` has `toint` but no float parse — `str.tofloat` was removed and has no replacement, so any program reading a float from stdin (e.g. the mortgage CLI tutorial) can't. Worked around in the mortgage example with a hand-rolled `parsefloat` (split on `.`, parse halves). Add `str.tofloat(s:$str):f64!$err`. |
| 114.40 | Tooling: no single-command multi-module binary build | planned | **P1** | `tkc a.tk b.tk c.tk -o bin` (per the docs/READMEs) fails — `-o` is an **unknown flag**, and `--out <dir> a.tk b.tk` emits **one binary per file** (cross-module symbols then undefined: `_mortgage_calc_calculate` referenced but not linked). The only working multi-module build is manual: `--emit-llvm` per module → `clang` link with the `--emit-deps` C sources (see `examples/mortgage/build.sh`). Either implement `-o`/single-binary multi-file linking, or ship a `tkc build` project command. Blocks every multi-module example/app. |
| 114.41 | **Discriminated sum-type matching is unimplemented** (not a small bug — a language feature) | planned | **P1** | Root cause diagnosed: a multi-variant sum type `t=$shape{$circle:f64;$rect:f64}` is represented as an **untagged struct with one field per variant** (IR shows `.circle`=field0, `.rect`=field1, no tag), and `mt` on it falls through to the **ok/err-by-zeroness** 2-arm bifurcation (`br i1 sh!=0, rm_ok, rm_err`) instead of dispatching on the active variant — so `mt s {$circle:r ..;$rect:r ..}` mis-compiles (`%t4 ptr vs double`) and a single-arm `mt e {$bad:msg ..}` references an unemitted `%rm_err` label. Consequently `T!$E` error unions returning `$E{$variant:val}` can't carry/inspect the variant. Proper fix = real **discriminated unions**: a tag in the sum-type rep, construction sets the tag, `mt` dispatches on it, each arm binds its variant's payload — a multi-day codegen/type-system feature, NOT a one-line fix. Until then, examples avoid sum-type errors (see `tkgrep`: pre-checks + default `mt {$ok:v v;$err:e ""}`). `str.toint`/`str.tofloat`-style single-payload `T!ParseErr` (ok-or-0) DO work. |
