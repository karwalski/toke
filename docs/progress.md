...
| 124.4a | Capability broker core (`src/stdlib/broker.c`) | planned | **P0** | Five gate classes — `fs.read`/`fs.write`/`net`/`env.write`/`process.spawn`; fail-closed. Route every sink (`os.c`/`file.c`/`env.c`/`process*.c`/net paths) through the broker. |
| 124.4b | Capability manifest format + parser | planned | **P0** | `[capabilities]` in `tkc.toml` (extend `src/config.c`) + `--allow-read/write/net/run/env` CLI flags (`src/main.c`); precedence CLI > tkc.toml > compiled. Narrow grants terse; `--allow-all` deliberately ugly/greppable. |
| 124.4c | Bake grant set into the `.tkc.md` companion | planned | P1 | Extend `src/companion.c` to record a program's required grants, hash-bound to source — "what can this binary touch" becomes a verifiable, declared fact. |
| 124.4d | RTcap structured missing-grant diagnostic | planned | **P0** | On a denied call, emit a schema-conforming diagnostic (per ADR-0001 §4) naming the exact missing grant, so the generate-compile-repair loop repairs it mechanically (like a type error). Prerequisite for the deny flip not hurting Pass@1. |
| 124.4e | First-party capability manifests | planned | **P0** | ooke, the website, and stdlib self-tests declare their real footprint (webroot read, listen socket, cert/key paths, DB socket, any `process.spawn`). Validates the broker (the compressed "Option B" proof) before the flip. |
| 124.4f | Grant-aware corpus/eval harness | planned | **P0** | Per-program manifests or a `--allow-all` test mode in the corpus/eval harness so I/O tasks aren't failed closed. **Hard prerequisite for the deny flip (124.6d)** — without it every I/O task's Pass@1 → ~0. Reaches into the `toke-model/corpus` harness. |
| 124.4g | Concrete ambient/toolchain defect fixes | planned | **P0** | Folds **121.15/121.22 + 122.10**: argv-vector exec default, `lstat` `rmdir_r`, containment `path_join`, `O_NOFOLLOW`, dotenv key allowlist, PATH snapshot, no raw int-as-pointer `os.read`/`write`; route the compiler's own child-`clang` through a grant (closes 120.2 COM-01/02). Worth doing independent of the flip. |
