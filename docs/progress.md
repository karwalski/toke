...
| 124.1 | **Wave A:** ADR-0013 crypto agility (full) | planned | **P0** | Versioned algorithm-tagged envelope in `encrypt.c`/`.h` (`version=0` legacy path); JWT alg-registry + allowlist + reject `none` (`auth.c:75`); hybrid `X25519MLKEM768` KEX (`tls.c:189`); committed signature PQC (ML-DSA/SLH-DSA) library-backed. Rides the 121.20 point-fixes. **Cleanest — no codegen/corpus risk; land first.** |
