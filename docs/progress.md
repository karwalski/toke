...
| 124.4h | Ambient defect hardening (post-flip) | planned | P1 | Independent of the capability flip (ADR-0010: "worth doing under any option"). Split from 124.4g. Open: AMB-05 `lstat`-based `rmdir_r` (`file.c:248`), AMB-06 normalizing containment `path_join` (`path.c:21`), AMB-07 `O_NOFOLLOW` on file ops, AMB-08 no raw int-as-pointer `os.read/os.write` (`os.c:38`). Re-scope first: AMB-03 (PATH snapshot) / AMB-04 (dotenv `LD_*`/`PATH` allowlist) are audit-**REFUTED** (remote reachability unsubstantiated). |
