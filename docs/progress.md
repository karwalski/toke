...
| ID | Story | Status | Date | Notes |
|----|-------|--------|------|-------|
| 89.2.6 | Template module SIGBUS: cross-module Result type match extraction | backlog | 2026-05-17 | The crash is NOT in nested struct access (verified working). It's in the match expression that extracts the Ok value from a cross-module Result return. `mt result {$ok:v v;$err:e 0}` produces 0 (error path) even when tpllex succeeds. The SIGBUS in the full test happens when this 0 is dereferenced as a struct pointer. Root cause: Result type encoding across fastcc module boundary may corrupt the discriminant or payload. Minimal reproducer: call tpl.tpllex, match result, access .kind field → SIGBUS. Confirmed tpllex itself works (returns 0 when just called). |
